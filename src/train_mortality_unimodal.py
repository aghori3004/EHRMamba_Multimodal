...
def main():
    print(f"🚀 Starting UNIMODAL Mortality Fine-Tuning on {DEVICE}")
    ...
    print("   Loading Unimodal Weights...")
    # Load the UNIMODAL Pre-trained weights
    model.load_state_dict(torch.load(CHECKPOINT_DIR / 'model_unimodal.pth'), strict=False)
    ...
    for epoch in range(FINE_TUNE_EPOCHS):
        ...
        for batch in pbar:
            ...
            with torch.cuda.amp.autocast(dtype=dtype):
                # FORCE UNIMODAL: Pass note_tensor=None
            ...
            # Metric tracking (Float32 cast)
            if logits.ndim == 0: 
                preds = [torch.sigmoid(logits).detach().float().cpu().item()]
                targs = [targets.float().cpu().item()]
            else: 
                preds = torch.sigmoid(logits).detach().float().cpu().numpy().tolist()
                targs = targets.float().cpu().numpy().tolist()
        ...
        torch.save(model.state_dict(), CHECKPOINT_DIR / 'model_mortality_unimodal.pth')
